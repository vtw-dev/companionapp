...
public static class EasyWiFiControlsMenu
{
    ...
    [MenuItem("GameObject/Easy WiFi Client Controls/Scrollable Button")]
    static void AddScrollableButton()
    {
        GameObject panel = getCanvasPanel();

        if (GameObject.Find("EventSystem") == null)
            new GameObject("EventSystem", typeof(UnityEngine.EventSystems.EventSystem), typeof(UnityEngine.EventSystems.StandaloneInputModule), typeof(UnityEngine.EventSystems.TouchInputModule));

        Sprite buttonRegularSprite = Resources.Load<Sprite>("buttonSprite");
        GameObject button = new GameObject("ScrollableButton", typeof(Image), typeof(ScrollableButtonClientController));
        RectTransform buttonRectTF = button.GetComponent<RectTransform>();
